...
// documentFileRepository is a concrete repository that implements repository.
type documentFileRepository struct {
	*repository
}

// NewDocumentRepository is constructor for documentRepository.
func NewDocumentFileRepository(logger logger.Logger, conf *config.Config, db *gorm.DB) Repository {
	return &documentFileRepository{&repository{db: db}}
}
...
func (rep *repository) Delete(value interface{}, conds ...interface{}) *gorm.DB {
	return rep.db.Delete(value, conds)
}
...
// AutoMigrate run auto migration for given models, will only add missing fields, won't delete/change current data
func (rep *repository) Session(config *gorm.Session) *gorm.DB {
	return rep.db.Session(config)
}

func (rep *repository) Begin(opts ...*sql.TxOptions) *gorm.DB {
	return rep.db.Begin()
}
